...
'''
# OLD VERSION - DOESN'T WORK because was trying to index with a boolean so I changed to an if statement.
...
# MY NEW VERSION TRYING TO FIX AN ISSUE - THIS WORKS !!
def ux_ss(xi, eta, q, dip, nu):
    R = np.sqrt(xi ** 2 + eta ** 2 + q ** 2)
    u = xi * q / (R * (R + eta)) + \
        I1(xi, eta, q, dip, nu, R) * np.sin(dip)
...
def uy_ds(xi, eta, q, dip, nu):
    ...
    if q != 0:
        u = u + np.cos(dip) * np.arctan( (xi * eta) / (q * R))
    return u
...
def uz_ds(xi, eta, q, dip, nu):
    ...
    if q != 0:
        ...
        u = u + np.sin(dip) * np.arctan( (xi * eta) / (q * R))
    return u
